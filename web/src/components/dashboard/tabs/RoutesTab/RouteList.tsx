...
export function RouteList({ routes, onEdit, onToggleStatus }: RouteListProps) {
  if (routes.length === 0) {
    return (
      <div className="bg-white/[0.02] border border-white/5 rounded-3xl p-20 flex flex-col items-center justify-center text-center">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-white/5 bg-white/[0.02]">
              <th className="p-5 text-[10px] font-bold text-gray-500 uppercase tracking-widest whitespace-nowrap">ID</th>
              <th className="p-5 text-[10px] font-bold text-gray-500 uppercase tracking-widest">Descrição / Nome</th>
              <th className="p-5 text-[10px] font-bold text-gray-500 uppercase tracking-widest">Periodicidade</th>
              <th className="p-5 text-[10px] font-bold text-gray-500 uppercase tracking-widest text-center">Clientes</th>
              <th className="p-5 text-[10px] font-bold text-gray-500 uppercase tracking-widest text-right">Ações</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-white/[0.02]">
            {routes.map((route) => (
              <tr key={route.id} className={`group hover:bg-white/[0.02] transition-colors ${!route.active ? 'opacity-50 grayscale' : ''}`}>
                <td className="p-5">
                  <span className="px-3 py-1 bg-white/5 rounded-lg text-[10px] font-mono font-bold text-purple-400 border border-white/5">
