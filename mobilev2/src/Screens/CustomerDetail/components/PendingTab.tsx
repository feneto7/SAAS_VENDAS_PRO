...
export const PendingTab = ({ clientId, clientName, collectionId }: Props) => {
  const { colors, isDark } = useTheme();
  const UI = useMemo(() => getUIStyles(colors, isDark), [colors, isDark]);
  const styles = useMemo(() => getStyles(colors, isDark), [colors, isDark]);

  const { navigate } = useNavigationStore();
  const { items, loading, refreshing, refresh } = useCardData(clientId, 'pendente');
    ...
    <FlatList
      ...
      ListEmptyComponent={
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptyText}>Nenhuma card pendente encontrada.</Text>
        </View>
      }
    />
  );
